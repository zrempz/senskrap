...
class ListParser(TwitcastingParser):
    ...
    def parse(self, html: str) -> Tuple[List[str], int]:
        ...
        links = self.extract_urls(
            soup.select(self._THUMBNAIL_LIST_SELECTOR), base_url=self._BASE_URL
        )
...
class ItemParser(TwitcastingParser):
    ...
    def parse(self, html: str, url: str, strip: bool = False) -> PremierItem:
        ...
        available_until_raw = self.extract_text(
            soup.select_one(self._AVAILABLE_UNTIL_SELECTOR)
        )
        archive_sales_raw = self.extract_text(
            soup.select_one(self._ARCHIVE_DEADLINE_SELECTOR)
        )
        tickets = []
        for container in soup.select(self._TICKET_CONTAINER_SELECTOR):
            ticket_title = self.extract_text(
                container.select_one(self._TICKET_TITLE_SELECTOR)
            )
            ticket_price_raw = self.extract_text(
                container.select_one(self._TICKET_PRICE_SELECTOR)
            )
            ticket_price = (
        ...
        return PremierItem(
            ...
            author_url=(
                urljoin(self._BASE_URL, str(url_author_elem.get("href", "")))
                if url_author_elem
                else None
            ),
            date=self.extract_text(soup.select_one(self._DATE_SELECTOR)),
            description=self.extract_with_line_breaks(
                soup.select_one(self._DESCRIPTION_SELECTOR), strip=strip
            ),
            image_url=(
                urljoin(self._BASE_URL, str(image_element.get("src", "")))
                if image_element
                else None
            ),
            available_until=(
                available_until_raw.replace("Available Period", "").strip()
                if available_until_raw
                else None
            ),
            archive_sales_deadline=(
                archive_sales_raw.replace("Archive sales deadline:", "").strip()
                if archive_sales_raw
                else None
            ),
            tickets=tickets,
        )
